...
use crate::logic::data_structures::patches::median_histogram_patch::MedianHistogramPatch;
...
fn compute_median(image: &DynamicImage, empty_image: &mut DynamicImage, lookup_table: &LookupTable,
    ...
    for v in 0..dimensions.1 {
        for u in 0..dimensions.0 {
            ...
            set_pixel_values(&mut pixel_value, &image.get_pixel(u as u32, v as u32).0, &median_patch, filter_input);
...
pub fn init_first_line(image: &DynamicImage, border_handling: &BorderHandling, position: &(i32, i32),
    ...
    for i in -vertical..=vertical {
        // remove
        let old_pixel_value = _non_linear_filter::get_pixel_value(image, border_handling, i, -patch_width, position, dimensions, filter_params);
        let old_rgba_average_value = get_average(&old_pixel_value, &filter_params.channels, lookup_table);

        // add
        let new_pixel_value = _non_linear_filter::get_pixel_value(image, border_handling, i, 0, position, dimensions, filter_params);
...
pub fn init_subsequent_start_position(image: &DynamicImage, border_handling: &BorderHandling, position: &(i32, i32),
    ...
    for j in -horizontal..=horizontal {
        let prev_pixel_value = _non_linear_filter::get_pixel_value(image, border_handling, -patch_height, j, position, dimensions, filter_params);
        ...
        let next_pixel_value = _non_linear_filter::get_pixel_value(image, border_handling, 0, j, position, dimensions, filter_params);
...
pub fn set_patch(image: &DynamicImage, border_handling: &BorderHandling, position: &(i32, i32),
    ...
    let prev_pixel_value = _non_linear_filter::get_pixel_value(image, border_handling, -patch_height, 0, position, dimensions, filter_params);
    let prev_rgba_average_value = get_average(&prev_pixel_value, &filter_params.channels, lookup_table);  // could be optimized with second lookup-table

    let next_pixel_value = _non_linear_filter::get_pixel_value(image, border_handling, 0, 0, position, dimensions, filter_params);
    let next_rgba_average_value = get_average(&next_pixel_value, &filter_params.channels, lookup_table);
...
fn set_pixel_values(pixel_value: &mut [u8; 4], original_pixel_value: &[u8; 4], patch: &MedianHistogramPatch, input_params: &FilterInput) {
    let new_value_red = if input_params.channels.red { patch.median_grayscale() } else { original_pixel_value[0] };
    let new_value_green = if input_params.channels.green { patch.median_grayscale() } else { original_pixel_value[1] };
    let new_value_blue = if input_params.channels.blue { patch.median_grayscale() } else { original_pixel_value[2] };
    let new_value_alpha = if input_params.channels.alpha { patch.median_grayscale() } else { original_pixel_value[3] };
