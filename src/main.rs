...
fn main() {
    argument_executor::execute(&App::new("Action")
        .version("0.1.0")
        .author("Alexander Mattheis <alexander.mattheis@web.de>")
        .about("Apply actions to images")
        .arg(Arg::with_name(commands::INPUT)
            .short("i")
            .long(commands::INPUT)
            .takes_value(true)
            .required(false)
            .long_help("The path to the image on which you want to apply a filter"))
        ...
        .arg(Arg::with_name(commands::ACTION)
            ...
            .takes_value(true)
            .required(false)
            .long_help("The action you want to apply on the image"))
        .arg(Arg::with_name(commands::PARAMS)
            .short("p")
            .long(commands::PARAMS)
            .takes_value(true)
            .required(false)
            .long_help("The data for the action you want to apply"))
        .arg(Arg::with_name(commands::OUTPUT)
            .short("o")
            .long(commands::OUTPUT)
            .takes_value(true)
            .required(false)
            .long_help("The output-path of the edited image"))
