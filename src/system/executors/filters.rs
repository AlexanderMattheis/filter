...
pub fn compute_box_blur(image: &DynamicImage, params: &String, output_file_name_path: &String) {
    let mut temp_image_1 = DynamicImage::new_rgb8(image.width(), image.height());
    let mut temp_image_2 = DynamicImage::new_rgb8(image.width(), image.height());
    let input_params = box_blur_parser::parse_params(params);
