...
use crate::system::defaults::output_filenames::ChannelOutputFilenames;
...
fn create_and_save_statistics(statistics_output: &StatisticsOutput, output_filepath_prefix: &String) {
    let mut file = File::create(strings::concat_with_static(&output_filepath_prefix, file_extensions::STATISTICS))
        .expect(errors::COULD_NOT_CREATE_FILE);

    let formatted_output = format!("# Statistics for:\n\
    **Minimum:** {minimum}\n\
    **Maximum:** {maximum}\n\
    **Average:** {average}\n\
    **Variance:** {variance}\n\
